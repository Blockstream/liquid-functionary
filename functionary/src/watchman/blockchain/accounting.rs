...
pub struct Account {
    ...
    controlled_funds: HashMap<bitcoin::OutPoint, u64>,
...
impl Account {
    ...
    pub fn requested_pegout_total(&self) -> u64 {
        self.requested_pegout
            .iter()
    ...
    pub fn pegged_in(&self) -> u64 {
        self.pegged_in
    }

    /// Accessor for the total amount of non-user funds
    pub fn non_user_funds(&self) -> u64 {
        self.non_user_funds
    }

    /// Record a pegin of a specified amount
    pub fn pegin(&mut self, outpoint: bitcoin::OutPoint, side_txid: elements::Txid, amount: u64) {
        ...
        slog!(Pegin, outpoint: outpoint, txid: side_txid, amount: amount,
            pegged_in: self.pegged_in, n_controlled_outputs: self.controlled_funds.len()
        );
    }
    ...
    pub fn pegout(&mut self, outpoint: elements::OutPoint, output: bitcoin::TxOut) {
        if self.pegged_in < output.value {
            slog_fatal!(ExcessPegout, outpoint: outpoint, amount: output.value,
                available: self.pegged_in,
            );
        }
        self.pegged_in -= output.value;
        slog!(Pegout, outpoint: outpoint, destination: &output, pegged_in: self.pegged_in,
            n_requested_pegouts: 1 + self.n_requested_pegouts(),
            requested_total: output.value + self.requested_pegout_total()
        );
    ...
    pub fn fee_donation(&mut self, outpoint: bitcoin::OutPoint, amount: u64) -> u64 {
        if let Some(old_amount) = self.controlled_funds.insert(outpoint, amount) {
            assert_eq!(old_amount, amount);
        ...
        let correction;
        if discrepancy < 0 {
            correction = cmp::min(amount, -discrepancy as u64);
        } else {
            correction = 0;
        }
        ...
        slog!(Donation, outpoint: outpoint, amount: amount, non_user_funds: self.non_user_funds,
                n_controlled_outputs: self.controlled_funds.len()
        );
        if correction != 0 {
            slog!(DiscrepancyCorrection, outpoint: outpoint, correction: correction,
                discrepancy: discrepancy + correction as i64
            );
        }
    ...
    pub fn fee_burn(&mut self, outpoint: elements::OutPoint, amount: u64) {
        if self.pegged_in < amount {
            slog_fatal!(ExcessBurn, outpoint: outpoint, amount: amount, available: self.pegged_in);
        }

        self.non_user_funds += amount;
        self.pegged_in -= amount;
        slog!(Burn, outpoint: outpoint, amount: amount, pegged_in: self.pegged_in,
            non_user_funds: self.non_user_funds
        );
    }
    ...
    pub fn finalize_federation_tx(
        ...
        for input in &tx.input {
            if let Some(amount) = self.controlled_funds.remove(&input.previous_output) {
                input_amount += amount;
                slog!(FinalizeSpend, outpoint: input.previous_output, txid: txid, amount: amount,
                    n_controlled_outputs: self.controlled_funds.len()
                );
            } else {
        ...
        for (n, output) in tx.output.iter().enumerate() {
            ...
            if Some(&output.script_pubkey) == change_spk {
                ...
                slog!(Change, outpoint: outpoint, amount: output.value,
                    n_controlled_outputs: self.controlled_funds.len()
                );
                ...
                slog!(FinalizePegout, outpoint: outpoint, destination: output,
                    ...
                    requested_total: self.requested_pegout_total()
                );
            } else {
        ...
        slog!(FinalizeFee, txid: txid, fee: fee, non_user_funds: self.non_user_funds);
    }
    ...
    pub fn discrepancy(&self) -> i64 {
        let controlled = self.controlled_funds.values().cloned().sum::<u64>();
        ...
        controlled as i64 - pegged_in as i64
    }

    /// Log all accounting data
    pub fn log_status(&self) {
        slog!(AccountingStatus,
            pegged_in: self.pegged_in,
            non_user_funds: self.non_user_funds,
            n_controlled_outputs: self.controlled_funds.len(),
            controlled_total: self.controlled_funds.values().copied().sum(),
            n_requested_pegouts: self.n_requested_pegouts(),
            requested_total: self.requested_pegout_total(),
